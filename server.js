...
async function sendMatchLifecycleEmail({match,emailType,recipientRole}={}){
  ...
  if(!match||!participant||!supporter||!['challenger','supporter'].includes(recipientRole)) return {skipped:true,reason:'match recipient unavailable'};
  ...
  if(!recipientEmail) return {skipped:true,reason:'recipient email unavailable'};
  ...
  const result=await sendFclResendEmail({
    ...
    text:bodyMap[emailType]||bodyMap.matching_candidate,
    html:'<div style="font-family:Arial,sans-serif;line-height:1.7"><div style="white-space:pre-wrap">'+escapeHtmlServer(bodyMap[emailType]||bodyMap.matching_candidate)+'</div><p style="margin-top:20px"><a href="'+escapeHtmlServer(detailUrl)+'" style="display:inline-block;padding:12px 18px;background:#111827;color:#fff;text-decoration:none;border-radius:8px">FCLの接続ページを開く</a></p></div>',
    idempotencyKey:'fcl/lifecycle-email/'+match.id+'/'+emailType+'/'+recipientRole
  });
  await insert('connection_events',{
    participant_id:match.participant_id,supporter_id:match.supporter_id,match_id:match.id,event_type:result?.skipped?'email_skipped':'email_sent',
...
async function buildSupporterAiSummaries({supporter,targets=[]}={}) {
  ...
  const cases=prepared.filter(x=>{ const existing=existingByMatch.get(x.target.match.id); return !existing || existing.source_fingerprint!==x.fingerprint || (existing.model_version!=='v3-ai' && Boolean(process.env.OPENAI_API_KEY)); });
...
app.post('/api/matches/:id/challenger-approve', async (req, res) => {
  try {
    ...
    const lifecycleEmail=await notifyMatchParties(updated, effectiveMatchStatus(updated)==='connected'?'connection_confirmed':'approval_received', effectiveMatchStatus(updated)==='connected'?['challenger','supporter']:['supporter']);
...
app.post('/api/matches/:id/supporter-approve', async (req, res) => {
  try {
    ...
    const lifecycleEmail=await notifyMatchParties(updated, effectiveMatchStatus(updated)==='connected'?'connection_confirmed':'approval_received', effectiveMatchStatus(updated)==='connected'?['challenger','supporter']:['challenger']);
